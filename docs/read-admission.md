...
over-budget frame is a typed policy refusal, not a crash-tail boundary: writer-open tail handling
will not truncate valid published bytes merely because the process was reopened under a stricter profile.
...
as an additional finalization target. If the next piece would cross it, the current non-empty block finalizes
first; small records therefore continue making progress under a strict profile. One indivisible
piece larger than either ceiling is refused before fold mutation. Publication, part merge, and refold
check every output part section and TOC before landing the completeness footer, so
...
pending change set or part-merge run exceeds policy, that operation returns `RESOURCE_EXHAUSTED` and leaves no
published output. The caller can use a larger read profile, flush more frequently, or select smaller
bounded part-merge inputs. TurnDB does not silently split one logical column into an undocumented
format extension.

## API surfaces

Rust embedders set `StoreOptions::read_limits`. Explicit variants are also available for `Part`,
`Fold`, container read views, and retained-manifest read views. Convenience opens
retain the defaults. `Store::read_limits`, `ReadStore::read_limits`, and writer `health()` expose the
effective policy.

Native Node writer and independently opened read-view options accept the two bigint fields. Defaults
are present in `capabilities()`, and writer `health()` reports effective values. Writer-created
read views inherit the writer's policy. Offline manifest promotion and staged backup restore accept the
...
profile, and returns effective values from `store.readLimits()`. Its one current `tdb_open` ABI
carries explicit values, with zero selecting compiled defaults.
...
or cache budgets. Manifests and candidate dictionaries have supported-reader ceilings; advisory fold sidecars derive their maximum from the
