...
TurnDB refuses to open a store whose current `MANIFEST` is missing or damaged, and a store whose
retained manifest history no longer validates: a retained revision a reader could open at is
authority, and authority the store cannot honour is not silently dropped. It never silently selects
an older retained manifest either, because doing so can abandon durability-acknowledged mutations.
Manifest promotion is the explicit, exclusive operator action for both, with a reported data-loss
bound and a reported retention loss.
...
2. Refuse a store whose current `MANIFEST` is intact and whose retained history validates.
   Manifest promotion cannot be used as an accidental rollback mechanism; an intact current
   manifest beside damaged retained history is promotable at rollback zero, which re-selects the
   same revision.
...
4. Examine retained manifest revisions from newest to oldest and find the nearest *fully usable*
   candidate. The candidate keeps the longest run of older retained revisions that are each valid,
   adjacent, linked by `prev`, monotonic in cursor and tail, and reopenable at their own tail. The
   first older revision that fails, and everything older than it, is abandoned: that ends time
   travel to those revisions and changes nothing else.
...
8. Publish with one container-state flip that installs the retained bytes as `MANIFEST`, drops the
   retained members of the abandoned newer timeline and of the abandoned older history, and removes
   part/fold members no surviving authority names, atomically.
...
The report includes the selected manifest revision, rollback distance, the number of older retained
revisions abandoned, records and content values validated, part/section counts, and fold
segment/block/byte counts.

The default allowance is zero. This permits replacement by the newest retained manifest revision—for example,
repairing bit rot in the current `MANIFEST` from its byte-identical retained copy—and, beneath an
intact current manifest, repairing damaged retained history by re-selecting the current revision
and abandoning the damaged copies. It refuses abandoning any newer retained manifest revision. Rollback distance is the numeric difference between the newest retained
