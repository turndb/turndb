...
TurnDB has three public ways to observe a record resolved through a manifest revision: point reads, storage-native structured
...
map applies every put and delete as the reference present/absent state. The store deliberately retains:
...
The gate first anchors every record ID whose slot resolves to a record or absence through point reads. It compares the exact ordered
...
DataFusion projects every scalar column and both content columns from the same immutable read view.
