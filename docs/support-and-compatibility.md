...
| Native macOS x86-64 and arm64 | GitHub's `macos-15-intel` (x86-64) and `macos-15` (Apple silicon) runners, real hardware: clippy, the debug and SQL-off suites, the corruption suite, the crash sweeps under both durability models, the release-profile suite, and the reference store byte-compared in both directions against the Linux x86-64 fixture — required gates on every push and pull request, so the macOS arms of `src/sys.rs` (`renamex_np`, the Darwin `statvfs` widths) are suite-tested, not merely driven. The `@turndb/cli` slices are built on the same runners, installed from their packed tarballs and driven, in CI and in the release install matrix. No native Node or Python package is built for macOS; the portable package serves it, with the capability difference it states. | supported and qualified for the Rust crate and `@turndb/cli` when those gates are green; native Node and Python remain open |
