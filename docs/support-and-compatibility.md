...
|---|---|---|
| Rust core, default and SQL-off | pinned stable Rust on GitHub's Linux x86-64 runner, and the same steps on its Linux arm64 (`ubuntu-22.04-arm`) and macOS (`macos-15-intel`, `macos-15`) runners; debug tests, clippy, rustdoc, the corruption suite and the release-profile suite all run hosted on every push and pull request (the release-profile link exceeded the private free-tier runner class while the repository was private; it has run hosted since the repository went public) | qualified development platform |
| Rust crash model | deterministic simulation under two durability models — strict POSIX, and Windows built from documented operations only (no directory fsync; write-through renames; a crash on a rename admitting old, new, or neither; unlinks never durable) — both models run on every platform: nightly on Linux x86-64, and on every push on Windows x86-64, Linux arm64, and macOS x86-64 and arm64 as required gates. The harness also fails every attempted sync of every publication sweep once, under both models, and requires the operation to report the failure and the store to converge (see "Sync failures" below) | qualified durability model on both platforms when those gates are green |
...
| Native Node | source-built addon plus Linux x86-64 glibc, Linux arm64 glibc (cross-built to the same GLIBC 2.17 floor, installed and exercised on `ubuntu-22.04-arm`), macOS x86-64 and Apple silicon (built and exercised on `macos-15-intel` and `macos-15`) and Windows x86-64 MSVC candidates, each installed from its packed tarballs on its own hardware — Linux and macOS on Node 22, 24, and 26 | release candidate after the matrices are green; tracked manifests remain private and registry status is owner-approved |
| Python SDK | PyO3 actor binding built and conformance-tested on CPython 3.12 on Linux x86-64, Linux arm64 and both macOS architectures; the release workflow builds manylinux x86-64 and aarch64 wheels (the latter in the `manylinux2014_aarch64` container on arm64 hardware), macOS x86-64 and Apple-silicon wheels (macOS 11 or newer), and Windows wheels, for CPython 3.9–3.13, and installs and exercises each exact wheel on the hardware it claims. Ships **without** the columnar/Arrow lens, SQL, and cooperative cancellation: `turndb.capabilities()` reports `columnar: false`, `arrowIpc: false`, `sql: false`, `cancellation: {scan: false, lifecycle: false}` | Linux x86-64 release candidate; a consumer that needs SQL or cancellation chooses the Rust crate or native Node |
...
| Native Linux arm64 glibc | GitHub's hosted `ubuntu-22.04-arm` runner, real hardware: clippy, the debug and SQL-off suites, the corruption suite, the crash sweeps under both durability models, the release-profile suite, and the reference store byte-compared in both directions against the x86-64 Linux fixture — all required gates on every push and pull request. The `@turndb/cli` slice for this architecture is built on that hardware, installed from its packed tarball and driven, both in CI and in the release install matrix; through 0.1.8 it was cross-compiled and published having never been executed. `src/sys.rs` has no arm64-specific arm — this qualifies the Unix arm on a second instruction set. The native Node addon for this architecture is cross-built to the same GLIBC 2.17 floor as x86-64 and installed and exercised on this hardware on Node 22, 24, and 26, in CI and in the native release path; it publishes with the next owner-approved release. Python wheels for this architecture are built in the `manylinux2014_aarch64` container on this hardware for CPython 3.9–3.13, and each exact wheel is installed and exercised here in the release path. | supported and qualified for the Rust crate and `@turndb/cli` when those gates are green; native Node and Python wheels prepared for the next release |
| Native macOS x86-64 and arm64 | GitHub's `macos-15-intel` (x86-64) and `macos-15` (Apple silicon) runners, real hardware: clippy, the debug and SQL-off suites, the corruption suite, the crash sweeps under both durability models, the release-profile suite, and the reference store byte-compared in both directions against the Linux x86-64 fixture — required gates on every push and pull request, so the macOS arms of `src/sys.rs` (`renamex_np`, the Darwin `statvfs` widths, and **punch** through `fcntl(F_PUNCHHOLE)` — exact on the bytes, with the unaligned head and tail of a range zeroed by write and the aligned interior deallocated; space return is measured, not promised) are suite-tested, not merely driven. The `@turndb/cli` slices are built on the same runners, installed from their packed tarballs and driven, in CI and in the release install matrix. The native Node addons for both architectures are built and exercised on these runners on Node 22, 24, and 26, in CI and in the native release path; they publish with the next owner-approved release. Python wheels for both architectures are built on these runners for CPython 3.9–3.13, and each exact wheel is installed and exercised here in the release path. | supported and qualified for the Rust crate and `@turndb/cli` when those gates are green; native Node and Python wheels prepared for the next release |
...
| `ReclaimStaging` | `<store>.reclaiming` | reclaim, either protocol, before the fresh container was published | removes it | refuses to create over it |
...
Qualified native package targets are Linux x86-64 and arm64 (glibc 2.17 or newer), macOS x86-64 and
Apple silicon, and Windows x86-64 MSVC — the three added on 2026-09-02 publish with the next
owner-approved release. musl and other native Unix systems remain unqualified even if a source build
