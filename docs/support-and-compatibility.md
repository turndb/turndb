...
|---|---|---|
| Rust core, default and SQL-off | pinned stable Rust on GitHub's Linux x86-64 runner; debug tests, clippy, rustdoc, corruption suite run hosted. The release-profile suite exceeded the private free-tier runner class (SIGBUS/disk, documented in the CI workflow); it armed hosted on the larger public runner when the repository went public on 2026-08-05, gated on `!github.event.repository.private` rather than on anyone remembering | hosted, on the public runner class |
