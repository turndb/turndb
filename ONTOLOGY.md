...
2. What does synchronization guarantee, and what does it leave unpublished?
...
| Concept | Definition |
|---|---|
| **store** | One logical TurnDB database, including its current and retained manifest revisions and any writer-only pending change set. |
...
| **writer view** | The view produced by resolving the current manifest revision together with the pending change set. |
| **read view** | An immutable logical observation pinned to one manifest revision; its continued readability depends on the required physical bytes remaining available. |
| **durability frontier** | The boundary through the WAL up to which accepted mutations have received the required durability barrier. |
| **manifest revision** | One ordered manifest state, identified by its commit counter, whose exact bytes are the logical authority naming parts, fold generation and tail, sequence cursor, ancestry, and declared punched blocks. |
...
| **container state** | One atomically selectable physical state identified by a valid superblock sequence. |
...
| **published** | manifest revision or container state | It has been made selectable through a completed container superblock transition. |
| **current** | manifest revision or container state | It is the state selected for a newly opened ordinary read view. |
| **retained** | manifest revision | Its exact manifest bytes are preserved in the commit log for reopening; retention does not determine whether it is current. |
| **revision-reachable** | container member | A current or retained manifest revision requires the member. |
| **live-content-reachable** | piece or fold block | At least one record resolved in the current revision requires its content. |
...
| **readable** | read view | Every physical byte required by the pinned manifest revision remains available and passes the checks required to read it. |
| **settled** | store | The pending change set is empty and WAL replay would add no mutation to it. |
...
| **synchronization** | Applies the durability barrier that advances the durability frontier without publishing a manifest revision. |
| **publication** | Writes the required physical artifacts and atomically selects a new container state; it may also publish a new manifest revision. |
| **WAL replay** | Reconstructs the pending change set from complete, valid WAL input after resolving the current manifest revision. |
...
| **reclaim** | Copies every member named by the container directory selected by the current container state into a fresh container and publishes it at the store path. |
...
| read view | pinned to | manifest revision |
| manifest revision | visible in | read view pinned to it |
| current manifest revision | visible in | writer view |
...
| container state | selects | manifest revision |
...
| durable accepted mutation or mutation batch | evidenced by | durability acknowledgement |
...
5. Acceptance does not imply durability. Durability does not imply publication. Publication does
   not by itself make the store settled because redundant WAL input may remain until truncation.
6. Synchronization advances the durability frontier and does not change an existing read view.
...
8. A successful reader open across a publication resolves either the preceding published container
...
14. A read view's selected manifest revision never changes. Its readability can nevertheless fail
...
|---|---|
| **commit** without qualification | Use **manifest revision** for manifest authority, **container state** for physical authority, or **publication** for the transition. |
...
| **recovery** without qualification | Use **WAL replay** for mutation replay. Manifest promotion and interrupted-publication procedures must be named separately outside this ontology. |
| **sealed** as a store lifecycle state | No replacement. The current format flag is a delegated physical mechanism and creates no product-level state. |
| **sync** as a general completion promise | State whether the promise is **synchronization**, **publication**, or that the store is **settled**. |
...
| [`docs/record-model-v2.md`](docs/record-model-v2.md) | Detailed record and named-content representation. |
| [`docs/field-types-v4.md`](docs/field-types-v4.md) | Exact scalar types and their cross-runtime representations. |
...
There is currently no delegated operation registry. Method names, aliases, composites, cancellation
points, and binding coverage therefore cannot introduce semantic distinctions.
