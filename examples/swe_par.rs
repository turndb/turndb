...
fn prepare(line: &str) -> Option<Prepared> {
    ...
    let mut cur = 0usize;
    for (a, b) in spans {
        if a > cur { ops.push(PreOp::Lit(cur..a)); }
        ops.push(PreOp::Piece { hash: PieceHash::of(&body[a..b]), at: a..b });
        cur = b;
    }
    if cur < body.len() { ops.push(PreOp::Lit(cur..body.len())); }
...
fn main() -> anyhow::Result<()> {
    ...
    while let Some(a) = it.next() {
        match a.as_str() {
            ...
            "--verify-every" => verify_every = it.next().and_then(|s| s.parse().ok()).unwrap_or(verify_every),
            "--level" => level = it.next().and_then(|s| s.parse().ok()).unwrap_or(level),
            "--part-level" => part_level = it.next().and_then(|s| s.parse().ok()).unwrap_or(part_level),
    ...
    eprintln!("readers: {}  per-part: {per_part}  fold-level: {level}  part-level: {plevel}", fifos.len());
    ...
    for p in rx {
        if seen.contains(&p.id) { skipped += 1; continue; }
        ...
        if sample_this { samples.push((pending.len(), body)); }
        ...
        if pending.len() >= per_part {
            ...
            for (idx, orig) in &samples {
                ...
                if &pt.reconstruct(row, &fold)? != orig { anyhow::bail!("BYTE DRIFT for {}", r.id); }
                if pt.attrs(row)? != r.attrs { anyhow::bail!("ATTR DRIFT for {}", r.id); }
                verified += 1;
            }
            nparts += 1;
            pending.clear();
    ...
    println!();
    println!("{:<26}{:.3} GiB  ({:.1}% of store)", "hashes alone (32B)",
        distinct as f64 * 32.0 / (1024.0*1024.0*1024.0), distinct as f64 * 32.0 * 100.0 / total as f64);
