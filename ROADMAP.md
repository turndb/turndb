...
> **2026-08-03, superseded numbering.** The entries below narrate the branch as it happened, and
> three of its steps were separate on-disk format revisions (2: named content; 3: whole-value
> identities; 4: the complete scalar set). Before first publication those intermediates — written
> by nothing outside this branch — were collapsed: the final model is on-disk **version 2**, the
> single successor of the version 1 that `main` writes, and the intermediate decoders are gone.
> Where an entry says revision 3 or 4, read it as history, not as a format the tree still carries.
...
- A defined initial field type system, including at least: **implemented in format version 2:**
  - UTF-8 string.
...
a version-1 pack written by `main`'s own build is restored, preflighted, migrated one part across a
restart, and verified byte-exact through public Node methods — with whole-value identities honestly
reported unavailable, because migration never invents them. **Sustained qualification is implemented:**
...
- Format-version compatibility fixtures retained across releases. **Initial retained fixture
  implemented:** a version-1 pack generated by `main`'s own writer, with recorded provenance.
- Upgrade fixtures containing stores written by older versions. **Implemented for version 1:** the
  public Node seam restores and migrates the retained pack byte-exact across restart.
